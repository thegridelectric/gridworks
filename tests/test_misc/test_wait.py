...
from gridworks_test import AwaitablePredicate
from gridworks_test import StopWatch
...
class Delay:
    ...
    def __init__(
        self, delay: float, result: bool = True, duration: Optional[float] = None
    ):
        self.result = result
...
async def test_await_for():
    ...
    with sw:
        await await_for(
            Delay(delay_time, duration=duration), timeout, retry_duration=retry_duration
        )
    ...
    with pytest.raises(ValueError):
        await await_for(Delay(1), 0.01)

    # Timeout exceeded, return result rather than raising exception
    assert await await_for(Delay(1, True), 0.01, raise_timeout=False) is False
    ...
    with sw:
        await await_for(
            async_delay(Delay(delay_time)), timeout, retry_duration=retry_duration
        )
    assert sw.elapsed >= delay_time
    assert sw.elapsed < min_expected
    assert sw.elapsed < timeout
